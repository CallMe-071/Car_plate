...
if __name__ == '__main__':
    ...
    index = 0
    for i in range(0 + 10, Num + 10):
        dir = 'train_images/training-set/letters/%s/' % i  # 将同一类型的图片存储在一个文件夹中进行遍历
        for rt, dirs, files in os.walk(dir):
            for filename in files:
    ...
    Val_count = 0
    for i in range(0 + 10, Num + 10):
        dir = 'train_images/validation-set/letters/%s/' % i
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                Val_count += 1
    ...
    index = 0
    for i in range(0 + 10, Num + 10):
        dir = 'train_images/validation-set/letters/%s/' % i  # 这里可以改成你自己的图片目录，i为分类标签
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                filename = dir + filename
                img = Image.open(filename)
    ...
    with tf.Session() as sess:
        ...
        print("总共有 %s 个训练图像， %s 个标签" % (input_count, input_count))
        ...
        plt.title('City_Province_Accuracy_Image')
        plt.xlabel('Iteration')
        plt.ylabel('Accuracy')
        plt.subplot(2, 1, 2)
        plt.plot(pic_x, loss, color='blue')
        plt.title('City_Loss_Image')
        plt.xlabel('Batch')
        plt.ylabel('Loss')
        # 显示准确率图标
        plt.show()
